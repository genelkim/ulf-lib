...
(defparameter *semtypes*
  (mapcar
    ...
    '(("{S}\\.PRO" . "D")  ; pronouns
      ...
      ;; TODO: update this comment based on what actually ends up working
      ;; TENSE  == {(VERB_!T>>VERB_T)|(AUX=>TAUX)}
      ;;        == {(*_V_!T%>*_V_T)|(((D=>(S=>2))_V_!T=>(D=>(S=>2))_V_!T,X)=>((D=>(S=>2))_V_U=>(D=>(S=>2))_V_T,X))}
      ;;        == {(({D|(D=>(S=>2))}^n=>(D=>(S=>2)))_V_!T=>({D|(D=>(S=>2))}^n=>(D=>(S=>2)))_V_T)
