...
(defparameter *semtypes*
  (mapcar
    ...
    '(("{S}\\.PRO" . "D")  ; pronouns
      ...
      ;; AUX    == ((D=>(S=>2))_V_!T_!X=>(D=>(S=>2))_V_!T_X)
      ;; TAUX   == ((D=>(S=>2))_V_!T_!X=>(D=>(S=>2))_V_T_X)
      ;;
      ;; TENSE  == {(*_V=>*_V_T)|(AUX=>TAUX)}
      ;;        == {(*_V=>*_V_T)|(((D=>(S=>2))_V_U=>(D=>(S=>2))_V_U_X)=>((D=>(S=>2))_V_U=>(D=>(S=>2))_V_T_X))}
      ;("{S}\\.AUX-S" . "((D=>(S=>2))_v_!t_!x=>(D=>(S=>2))_v_!t_x)")
      ;("{S}\\.AUX-V" . "((D=>(S=>2))_v_!t_!x=>(D=>(S=>2))_v_!t_x)")
      ("{S}\\.AUX-S" . "((D=>(S=>2))_v_!x=>(D=>(S=>2))_v_x)")
      ("{S}\\.AUX-V" . "((D=>(S=>2))_v_!x=>(D=>(S=>2))_v_x)")
      ("{S}\\.P-ARG" . "PARG")
      ("\\!" . "((S=>2)>>(S=>2))")
      ("\\?" . "((S=>2)>>(S=>2))")
      )))
