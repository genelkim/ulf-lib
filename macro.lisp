...
(defun contains-hole (inulf &key (inholevar '*h))
  (in-intern (inulf ulf :ulf-lib)
    (in-intern (inholevar holevar :ulf-lib)
                     (cond
       ...
       (let ((initial-result
         ...
         (if calling-package
           (values (first initial-result)
                   (intern-symbols-recursive (second initial-result) calling-package))
...
(defun add-info-to-relativizer (curulf srculf)
  (let* ((origrel (first (tree-find-if curulf #'lex-rel?)))
         (replacement
...
(defun apply-qt-attr-macro (inulf &key (calling-package nil))
  (labels
    ...
    (let
      ((initial-result (multiple-value-list (in-intern
                                              (inulf ulf :ulf-lib)
                                              (rec-helper ulf)))))
      (cond
        ...
        (calling-package  (values (first initial-result)
                                  (intern-symbols-recursive (second initial-result)
                                                            calling-package)))
